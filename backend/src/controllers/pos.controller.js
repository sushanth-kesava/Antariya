...
exports.getInvoices = async (req, res) => { try { const { startDate, endDate, customerPhone, status, page, limit } = req.query; const isSuperAdmin = req.auth?.role === 'superadmin'; const billedBy = isSuperAdmin ? null : req.auth.sub; const r = await POSService.getInvoices({ startDate, endDate, customerPhone, status, page: parseInt(page) || 1, limit: parseInt(limit) || 20, billedBy }); res.json({ success: true, data: r }); } catch (e) { res.status(500).json({ success: false, message: e.message }); } };
exports.getInvoiceById = async (req, res) => { try { const inv = await POSService.getInvoiceById(req.params.id); if (!inv) return res.status(404).json({ success: false, message: 'Not found' }); const isSuperAdmin = req.auth?.role === 'superadmin'; if (!isSuperAdmin && inv.billedBy?.toString() !== req.auth.sub) { return res.status(403).json({ success: false, message: 'Access denied' }); } res.json({ success: true, data: inv }); } catch (e) { res.status(500).json({ success: false, message: e.message }); } };
...
exports.getDashboard = async (req, res) => { try { const isSuperAdmin = req.auth?.role === 'superadmin'; const billedBy = isSuperAdmin ? null : req.auth.sub; const s = await POSService.getDashboard(billedBy); res.json({ success: true, data: s }); } catch (e) { res.status(500).json({ success: false, message: e.message }); } };
