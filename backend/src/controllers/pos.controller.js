...
exports.getInvoices = async (req, res) => { try { const { startDate, endDate, customerPhone, status, page, limit } = req.query; const isSuperAdmin = req.auth?.role === 'superadmin'; let productIds = null; if (!isSuperAdmin) { const prods = await Product.find({ dealerId: req.auth.sub }).select('_id'); productIds = prods.map(p => p._id); } const r = await POSService.getInvoices({ startDate, endDate, customerPhone, status, page: parseInt(page) || 1, limit: parseInt(limit) || 20, productIds }); res.json({ success: true, data: r }); } catch (e) { res.status(500).json({ success: false, message: e.message }); } };
exports.getInvoiceById = async (req, res) => { try { const inv = await POSService.getInvoiceById(req.params.id); if (!inv) return res.status(404).json({ success: false, message: 'Not found' }); const isSuperAdmin = req.auth?.role === 'superadmin'; if (!isSuperAdmin) { const prods = await Product.find({ dealerId: req.auth.sub }).select('_id'); const ids = prods.map(p => p._id.toString()); const hasOwn = inv.items.some(item => ids.includes(item.productId?.toString())); if (!hasOwn) return res.status(403).json({ success: false, message: 'Access denied' }); } res.json({ success: true, data: inv }); } catch (e) { res.status(500).json({ success: false, message: e.message }); } };
...
exports.cancelInvoice = async (req, res) => { try { const inv = await POSService.cancelInvoice({ invoiceId: req.params.id, reason: req.body?.reason, userId: req.auth.sub }); res.json({ success: true, data: inv }); } catch (e) { const code = e.message === 'Invoice not found' ? 404 : 500; res.status(code).json({ success: false, message: e.message }); } };
exports.getDashboard = async (req, res) => { try { const isSuperAdmin = req.auth?.role === 'superadmin'; let productIds = null; if (!isSuperAdmin) { const prods = await Product.find({ dealerId: req.auth.sub }).select('_id'); productIds = prods.map(p => p._id); } const s = await POSService.getDashboard(null, productIds); res.json({ success: true, data: s }); } catch (e) { res.status(500).json({ success: false, message: e.message }); } };
