        ...
        if (result.pendingApproval) {
          toast({
            ...
            description: result.message || "Your request is waiting for superadmin approval.",
              ...
              router.replace(buildAuthUrl("login", selectedRoleForGoogle, nextPath));
          return;
        }
  ...
  const handleCredentialsSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      toast({
        ...
        description:
          selectedRole === "superadmin"
            ...
            : "Superadmin signup now uses a dedicated application form.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (isPrivilegedCredentialsFlow) {
      ...
      toast({
        ...
        description:
          selectedRole === "superadmin"
                    ? "Superadmin access is available only through approved Google sign-in."
            ...
            {blockedPrivilegedSignup ? (
              <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 flex gap-2">
                ...
                <span>
                  ...
                  {selectedRole === "superadmin" ? (
                    <Link href="/superadmin-login/apply" className="font-semibold underline underline-offset-2">
                      Open the superadmin application page
                    </Link>
                  ) : null}
                </span>
              </div>
